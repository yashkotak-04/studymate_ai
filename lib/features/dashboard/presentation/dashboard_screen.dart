...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final String greeting = 'Good Morning, ${userProfile?.displayName?.split(' ').first ?? 'Student'} 👋';
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          todayDate,
                          style: AppTextStyles.bodySecondary(context, fontWeight: FontWeight.w500),
                        ),
                        const SizedBox(height: 2),
                        Text(
                          greeting,
                          style: AppTextStyles.displayBold(context, fontSize: 22),
                        ),
                      ],
                    ),
                  ),
                  Container(
                    ...
                    child: Text(
                      ...
                      style: AppTextStyles.displayBold(context, fontSize: 15, color: Colors.white),
                    ),
                  ),
                ],
              ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Row(
                        ...
                        children: [
                          Icon(LucideIcons.flame, color: AppColors.accentAmber, size: 26),
                          ...
                          Column(
                            ...
                            children: [
                              Text(
                                '$streak',
                                style: AppTextStyles.monoBold(context, fontSize: 20),
                              ),
                              Text(
                                'day streak',
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('Today\'s Goal', style: AppTextStyles.body(context, fontWeight: FontWeight.w600)),
                        Text('$completedMins / $totalMins mins', style: AppTextStyles.monoBold(context, fontSize: 13).copyWith(color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary)),
                      ],
                    ),
              ...
              Row(
                ...
                children: [
                  Text('Your subjects', style: AppTextStyles.displayBold(context, fontSize: 15)),
                  Text('${userProfile?.enrolledSubjectIds.length ?? 0} enrolled', style: AppTextStyles.bodySecondary(context, fontSize: 12)),
                ],
              ),
                ...
                child: Builder(
                  builder: (context) {
                    ...
                    if (enrolledIds.isEmpty) {
                      return const Center(child: Text('No subjects enrolled yet.'));
                    }
                    final enrolledSubjects = AppSubjects.availableSubjects.where((s) => enrolledIds.contains(s.id)).toList();
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (subjectProgress != null) {
                          final sp = subjectProgress.where((s) => s['id'] == subject.id).firstOrNull;
                          if (sp != null) {
                            progress = (sp['accuracy'] as num?)?.toDouble() ?? 0;
                          }
                        }
                    return InkWell(
                            ...
                            Column(
                              ...
                              children: [
                                ProgressRing(
                                  ...
                                  strokeWidth: 4,
                                  color: subject.color,
                                  child: Text('${progress.toInt()}', style: AppTextStyles.monoBold(context, fontSize: 11)),
                                ),
                                const Spacer(),
                                Text(
                                  subject.shortName,
              ...
              // Quick Actions
              Text('Quick actions', style: AppTextStyles.displayBold(context, fontSize: 15)),
              const SizedBox(height: 10),
              GridView.count(
                crossAxisCount: 2,
                shrinkWrap: true,
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Personalized Focus', style: AppTextStyles.displayBold(context, fontSize: 15)),
                  TextButton(
                    onPressed: () => context.push('/recommendations'),
                    child: Text('View all', style: AppTextStyles.body(context, fontSize: 12, color: AppColors.primary, fontWeight: FontWeight.w700)),
                  ),
                ],
              ),
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Text(
                                'AI STUDY ROADMAP',
                                style: AppTextStyles.body(context, fontSize: 11, fontWeight: FontWeight.w700, color: Colors.white).copyWith(letterSpacing: 0.5),
                              ),
                            ],
                          ),
                          const SizedBox(height: 6),
                          Text(
                            ...
                            style: AppTextStyles.body(context, fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white),
                          ),
                          const SizedBox(height: 4),
                          Text(
                            'View tailored study advice, practice sets, and timetable suggestions.',
                            style: AppTextStyles.body(context, fontSize: 13, color: Colors.white.withOpacity(0.85)),
                          ),
                        ],
                      ),
                    );
                  ...
                  return CustomCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            const Icon(LucideIcons.info, size: 13, color: Colors.white),
                        ...
                        Row(
                          children: [
                            Text('Start practice', style: AppTextStyles.body(context, fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white)),
                            const SizedBox(width: 4),
    ...
    return CustomCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(label, style: AppTextStyles.body(context, fontSize: 13, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
